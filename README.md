...
Google Animation
Enhanced 
WITH :colored stress strain tensor contours
...
[First Frame](https://github.com/rogerjdeangelis/utl-altair-slc-enhanced-openradioss-tensile-strength-simulation-python-no-gui-no-mouse-surfing/blob/main/frame_0001.png)

[Last Frame](https://github.com/rogerjdeangelis/utl-altair-slc-enhanced-openradioss-tensile-strength-simulation-python-no-gui-no-mouse-surfing/blob/main/frame_0041.png)

For validation, an exact reproduction of the published plot was created,
Published plot https://openradioss.atlassian.net/wiki/spaces/OPENRADIOSS/pages/24444938/Tensile+Test+Example+Tutorial+Using+Gmsh Result

My plot
Validation Result 

https://github.com/rogerjdeangelis/utl-altair-slc-python-script-to-run-openradioss-tensile-strength-simulation/blob/main/Fixed_End_RBODY_FX_X_FORCE.png
...
/*  ___         _   _      _                _   _       _  __
/ |/ _ \ __   _| |_| | __ | |_ ___   __   _| |_| | ____| |/ _|
| | | | |\ \ / / __| |/ / | __/ _ \  \ \ / / __| |/ / _` | |_
| | |_| | \ V /| |_|   <  | || (_) |  \ V /| |_|   < (_| |  _|
|_|\___/   \_/  \__|_|\_\  \__\___/    \_/  \__|_|\_\__,_|_|
...
if __name__ == "__main__":
    ...
    INPUT_FILE_PATTERN = "D:/rad/gmsh_tensile_LAW36_BIQUADA*.vtk"
    OUTPUT_FILE = "D:/rad/gmsh_tensile_LAW36_BIQUAD.vtkhdf"
...
/* _        _   _    _         _  __   _
/ / |__   _| |_| | _| |__   __| |/ _| | |_ ___     ___ _____   __
| | |\ \ / / __| |/ / `_ \ / _` | |_  | __/ _ \   / __/ __\ \ / /
| | | \ V /| |_|   <| | | | (_| |  _| | || (_) | | (__\__ \\ V /
|_|_|  \_/  \__|_|\_\_| |_|\__,_|_|    \__\___/   \___|___/ \_/
...
#!/usr/bin/env python
"""
Extract stress and strain data from VTKHDF.
...
# CONFIGURATION
# ------------------------------------------------------------
VTKHDF_FILE = "D:/rad/gmsh_tensile_LAW36_BIQUAD.vtkhdf"
OUTPUT_DIR = "D:/rad"
...
# ------------------------------------------------------------
# METHOD 1: EXTRACT CELL DATA (Stress/Strain) - CORRECTED
# ------------------------------------------------------------
print("\n--- Extracting Cell Data (Stress/Strain) ---")

# CORRECTED: Use CellDatatoPointData (no underscore after Cell)
cell_to_point = CellDatatoPointData(Input=source)

# Now save the cell data as CSV
output_cell = os.path.join(OUTPUT_DIR, "stress_strain_data.csv")
SaveData(output_cell, proxy=cell_to_point, WriteTimeSteps=1, Precision=12)
print(f"Stress/strain data saved to: {output_cell}")

# ------------------------------------------------------------
# METHOD 2: EXTRACT GLOBAL ENERGY DATA
# ------------------------------------------------------------
print("\n--- Extracting Global Energy Data ---")
integrate = IntegrateVariables(Input=source)
output_global = os.path.join(OUTPUT_DIR, "global_energy_data.csv")
SaveData(output_global, proxy=integrate, WriteTimeSteps=1, Precision=12)
print(f"Global energy data saved to: {output_global}")

# ------------------------------------------------------------
# METHOD 3: EXTRACT VON MISES STRESS
# ------------------------------------------------------------
print("\n--- Extracting Von Mises Stress ---")

# Use a programmable filter to compute Von Mises stress
# Since Calculator requires the exact array names, we'll use a simpler approach
# First, convert cell data to point data
cell_to_point_filter = CellDatatoPointData(Input=source)

# Save the converted data which includes all stress/strain arrays
output_converted = os.path.join(OUTPUT_DIR, "converted_stress_strain.csv")
SaveData(output_converted, proxy=cell_to_point_filter, WriteTimeSteps=1, Precision=12)
print(f"Converted stress/strain data saved to: {output_converted}")

# ------------------------------------------------------------
# VERIFY OUTPUTS
# ------------------------------------------------------------
print("\n--- Summary ---")
for f in ['stress_strain_data.csv', 'global_energy_data.csv', 'converted_stress_strain.csv']:
...
/* ____        _                           _             _        _
/ |___ \   ___| |_ _ __ ___  ___ ___   ___| |_ _ __ __ _(_)_ __  | |_ ___ _ __  ___  ___  _ __ ___
| | __) | / __| __| `__/ _ \/ __/ __| / __| __| `__/ _` | | `_ \ | __/ _ \ `_ \/ __|/ _ \| `__/ __|
| |/ __/  \__ \ |_| | |  __/\__ \__ \ \__ \ |_| | | (_| | | | | || ||  __/ | | \__ \ (_) | |  \__ \
|_|_____| |___/\__|_|  \___||___/___/ |___/\__|_|  \__,_|_|_| |_| \__\___|_| |_|___/\___/|_|  |___/
*/

options ls=255;

data workx.stress_strain_tensors;

  infile "d:/rad/converted_stress_strain.csv" delimiter=',' dsd missover lrecl=32756 firstobs=2;

/* Element identification */
label NODEID        = "Node Identification Number"
      ELEMENTID     = "Element Identification Number"
      PARTID        = "Part Identification Number"
      EROSIONSTATUS = "Element Erosion Status (0=active, 1=eroded)"

/* ----- STRAIN TENSOR COMPONENTS (Layer 1-5) ----- */
/* Layer 1 (Bottom integration point) */
    l TWODELEMStrainlayer10 = "Strain Tensor Component XX - Layer 1 (Bottom)"
      TWODELEMStrainlayer11 = "Strain Tensor Component YY - Layer 1 (Bottom)"
      TWODELEMStrainlayer12 = "Strain Tensor Component ZZ - Layer 1 (Bottom)"
      TWODELEMStrainlayer13 = "Strain Tensor Component XY - Layer 1 (Bottom)"
      TWODELEMStrainlayer14 = "Strain Tensor Component YZ - Layer 1 (Bottom)"
      TWODELEMStrainlayer15 = "Strain Tensor Component ZX - Layer 1 (Bottom)"
      TWODELEMStrainlayer16 = "Strain Component 6 - Layer 1 (Bottom)"
      TWODELEMStrainlayer17 = "Strain Component 7 - Layer 1 (Bottom)"
      TWODELEMStrainlayer18 = "Strain Component 8 - Layer 1 (Bottom)"

/* Layer 2 (Second integration point) */
      TWODELEMStrainlayer20 = "Strain Tensor Component XX - Layer 2"
      TWODELEMStrainlayer21 = "Strain Tensor Component YY - Layer 2"
      TWODELEMStrainlayer22 = "Strain Tensor Component ZZ - Layer 2"
      TWODELEMStrainlayer23 = "Strain Tensor Component XY - Layer 2"
      TWODELEMStrainlayer24 = "Strain Tensor Component YZ - Layer 2"
      TWODELEMStrainlayer25 = "Strain Tensor Component ZX - Layer 2"
      TWODELEMStrainlayer26 = "Strain Component 6 - Layer 2"
      TWODELEMStrainlayer27 = "Strain Component 7 - Layer 2"
      TWODELEMStrainlayer28 = "Strain Component 8 - Layer 2"

/* Layer 3 (Middle integration point) */
      TWODELEMStrainlayer30 = "Strain Tensor Component XX - Layer 3 (Middle)"
      TWODELEMStrainlayer31 = "Strain Tensor Component YY - Layer 3 (Middle)"
      TWODELEMStrainlayer32 = "Strain Tensor Component ZZ - Layer 3 (Middle)"
      TWODELEMStrainlayer33 = "Strain Tensor Component XY - Layer 3 (Middle)"
      TWODELEMStrainlayer34 = "Strain Tensor Component YZ - Layer 3 (Middle)"
      TWODELEMStrainlayer35 = "Strain Tensor Component ZX - Layer 3 (Middle)"
      TWODELEMStrainlayer36 = "Strain Component 6 - Layer 3 (Middle)"
      TWODELEMStrainlayer37 = "Strain Component 7 - Layer 3 (Middle)"
      TWODELEMStrainlayer38 = "Strain Component 8 - Layer 3 (Middle)"

/* Layer 4 (Fourth integration point) */
      TWODELEMStrainlayer40 = "Strain Tensor Component XX - Layer 4"
      TWODELEMStrainlayer41 = "Strain Tensor Component YY - Layer 4"
      TWODELEMStrainlayer42 = "Strain Tensor Component ZZ - Layer 4"
      TWODELEMStrainlayer43 = "Strain Tensor Component XY - Layer 4"
      TWODELEMStrainlayer44 = "Strain Tensor Component YZ - Layer 4"
      TWODELEMStrainlayer45 = "Strain Tensor Component ZX - Layer 4"
      TWODELEMStrainlayer46 = "Strain Component 6 - Layer 4"
      TWODELEMStrainlayer47 = "Strain Component 7 - Layer 4"
      TWODELEMStrainlayer48 = "Strain Component 8 - Layer 4"

/* Layer 5 (Top integration point) */
      TWODELEMStrainlayer50 = "Strain Tensor Component XX - Layer 5 (Top)"
      TWODELEMStrainlayer51 = "Strain Tensor Component YY - Layer 5 (Top)"
      TWODELEMStrainlayer52 = "Strain Tensor Component ZZ - Layer 5 (Top)"
      TWODELEMStrainlayer53 = "Strain Tensor Component XY - Layer 5 (Top)"
      TWODELEMStrainlayer54 = "Strain Tensor Component YZ - Layer 5 (Top)"
      TWODELEMStrainlayer55 = "Strain Tensor Component ZX - Layer 5 (Top)"
      TWODELEMStrainlayer56 = "Strain Component 6 - Layer 5 (Top)"
      TWODELEMStrainlayer57 = "Strain Component 7 - Layer 5 (Top)"
      TWODELEMStrainlayer58 = "Strain Component 8 - Layer 5 (Top)"

/* Lower surface strain (extrapolated) */
      TWODELEMStrainlower0 = "Strain Tensor Component XX - Lower Surface"
      TWODELEMStrainlower1 = "Strain Tensor Component YY - Lower Surface"
      TWODELEMStrainlower2 = "Strain Tensor Component ZZ - Lower Surface"
      TWODELEMStrainlower3 = "Strain Tensor Component XY - Lower Surface"
      TWODELEMStrainlower4 = "Strain Tensor Component YZ - Lower Surface"
      TWODELEMStrainlower5 = "Strain Tensor Component ZX - Lower Surface"
      TWODELEMStrainlower6 = "Strain Component 6 - Lower Surface"
      TWODELEMStrainlower7 = "Strain Component 7 - Lower Surface"
      TWODELEMStrainlower8 = "Strain Component 8 - Lower Surface"

/* Upper surface strain (extrapolated) */
      TWODELEMStrainupper0 = "Strain Tensor Component XX - Upper Surface"
      TWODELEMStrainupper1 = "Strain Tensor Component YY - Upper Surface"
      TWODELEMStrainupper2 = "Strain Tensor Component ZZ - Upper Surface"
      TWODELEMStrainupper3 = "Strain Tensor Component XY - Upper Surface"
      TWODELEMStrainupper4 = "Strain Tensor Component YZ - Upper Surface"
      TWODELEMStrainupper5 = "Strain Tensor Component ZX - Upper Surface"
      TWODELEMStrainupper6 = "Strain Component 6 - Upper Surface"
      TWODELEMStrainupper7 = "Strain Component 7 - Upper Surface"
      TWODELEMStrainupper8 = "Strain Component 8 - Upper Surface"

/* ----- STRESS TENSOR COMPONENTS (Layer 1-5) ----- */
/* Layer 1 (Bottom) Stress */
      TWODELEMStresslayer10 = "Stress Tensor Component XX - Layer 1 (Bottom) [MPa]"
      TWODELEMStresslayer11 = "Stress Tensor Component YY - Layer 1 (Bottom) [MPa]"
      TWODELEMStresslayer12 = "Stress Tensor Component ZZ - Layer 1 (Bottom) [MPa]"
      TWODELEMStresslayer13 = "Stress Tensor Component XY - Layer 1 (Bottom) [MPa]"
      TWODELEMStresslayer14 = "Stress Tensor Component YZ - Layer 1 (Bottom) [MPa]"
      TWODELEMStresslayer15 = "Stress Tensor Component ZX - Layer 1 (Bottom) [MPa]"
      TWODELEMStresslayer16 = "Stress Component 6 - Layer 1 (Bottom) [MPa]"
      TWODELEMStresslayer17 = "Stress Component 7 - Layer 1 (Bottom) [MPa]"
      TWODELEMStresslayer18 = "Stress Component 8 - Layer 1 (Bottom) [MPa]"

/* Layer 2 Stress */
      TWODELEMStresslayer20 = "Stress Tensor Component XX - Layer 2 [MPa]"
      TWODELEMStresslayer21 = "Stress Tensor Component YY - Layer 2 [MPa]"
      TWODELEMStresslayer22 = "Stress Tensor Component ZZ - Layer 2 [MPa]"
      TWODELEMStresslayer23 = "Stress Tensor Component XY - Layer 2 [MPa]"
      TWODELEMStresslayer24 = "Stress Tensor Component YZ - Layer 2 [MPa]"
      TWODELEMStresslayer25 = "Stress Tensor Component ZX - Layer 2 [MPa]"
      TWODELEMStresslayer26 = "Stress Component 6 - Layer 2 [MPa]"
      TWODELEMStresslayer27 = "Stress Component 7 - Layer 2 [MPa]"
      TWODELEMStresslayer28 = "Stress Component 8 - Layer 2 [MPa]"

/* Layer 3 (Middle) Stress */
      TWODELEMStresslayer30 = "Stress Tensor Component XX - Layer 3 (Middle) [MPa]"
      TWODELEMStresslayer31 = "Stress Tensor Component YY - Layer 3 (Middle) [MPa]"
      TWODELEMStresslayer32 = "Stress Tensor Component ZZ - Layer 3 (Middle) [MPa]"
      TWODELEMStresslayer33 = "Stress Tensor Component XY - Layer 3 (Middle) [MPa]"
      TWODELEMStresslayer34 = "Stress Tensor Component YZ - Layer 3 (Middle) [MPa]"
      TWODELEMStresslayer35 = "Stress Tensor Component ZX - Layer 3 (Middle) [MPa]"
      TWODELEMStresslayer36 = "Stress Component 6 - Layer 3 (Middle) [MPa]"
      TWODELEMStresslayer37 = "Stress Component 7 - Layer 3 (Middle) [MPa]"
      TWODELEMStresslayer38 = "Stress Component 8 - Layer 3 (Middle) [MPa]"

/* Layer 4 Stress */
      TWODELEMStresslayer40 = "Stress Tensor Component XX - Layer 4 [MPa]"
      TWODELEMStresslayer41 = "Stress Tensor Component YY - Layer 4 [MPa]"
      TWODELEMStresslayer42 = "Stress Tensor Component ZZ - Layer 4 [MPa]"
      TWODELEMStresslayer43 = "Stress Tensor Component XY - Layer 4 [MPa]"
      TWODELEMStresslayer44 = "Stress Tensor Component YZ - Layer 4 [MPa]"
      TWODELEMStresslayer45 = "Stress Tensor Component ZX - Layer 4 [MPa]"
      TWODELEMStresslayer46 = "Stress Component 6 - Layer 4 [MPa]"
      TWODELEMStresslayer47 = "Stress Component 7 - Layer 4 [MPa]"
      TWODELEMStresslayer48 = "Stress Component 8 - Layer 4 [MPa]"

/* Layer 5 (Top) Stress */
      TWODELEMStresslayer50 = "Stress Tensor Component XX - Layer 5 (Top) [MPa]"
      TWODELEMStresslayer51 = "Stress Tensor Component YY - Layer 5 (Top) [MPa]"
      TWODELEMStresslayer52 = "Stress Tensor Component ZZ - Layer 5 (Top) [MPa]"
      TWODELEMStresslayer53 = "Stress Tensor Component XY - Layer 5 (Top) [MPa]"
      TWODELEMStresslayer54 = "Stress Tensor Component YZ - Layer 5 (Top) [MPa]"
      TWODELEMStresslayer55 = "Stress Tensor Component ZX - Layer 5 (Top) [MPa]"
      TWODELEMStresslayer56 = "Stress Component 6 - Layer 5 (Top) [MPa]"
      TWODELEMStresslayer57 = "Stress Component 7 - Layer 5 (Top) [MPa]"
      TWODELEMStresslayer58 = "Stress Component 8 - Layer 5 (Top) [MPa]"

/* Lower surface stress (extrapolated) */
      TWODELEMStresslower0 = "Stress Tensor Component XX - Lower Surface [MPa]"
      TWODELEMStresslower1 = "Stress Tensor Component YY - Lower Surface [MPa]"
      TWODELEMStresslower2 = "Stress Tensor Component ZZ - Lower Surface [MPa]"
      TWODELEMStresslower3 = "Stress Tensor Component XY - Lower Surface [MPa]"
      TWODELEMStresslower4 = "Stress Tensor Component YZ - Lower Surface [MPa]"
      TWODELEMStresslower5 = "Stress Tensor Component ZX - Lower Surface [MPa]"
      TWODELEMStresslower6 = "Stress Component 6 - Lower Surface [MPa]"
      TWODELEMStresslower7 = "Stress Component 7 - Lower Surface [MPa]"
      TWODELEMStresslower8 = "Stress Component 8 - Lower Surface [MPa]"

/* Upper surface stress (extrapolated) */
      TWODELEMStressupper0 = "Stress Tensor Component XX - Upper Surface [MPa]"
      TWODELEMStressupper1 = "Stress Tensor Component YY - Upper Surface [MPa]"
      TWODELEMStressupper2 = "Stress Tensor Component ZZ - Upper Surface [MPa]"
      TWODELEMStressupper3 = "Stress Tensor Component XY - Upper Surface [MPa]"
      TWODELEMStressupper4 = "Stress Tensor Component YZ - Upper Surface [MPa]"
      TWODELEMStressupper5 = "Stress Tensor Component ZX - Upper Surface [MPa]"
      TWODELEMStressupper6 = "Stress Component 6 - Upper Surface [MPa]"
      TWODELEMStressupper7 = "Stress Component 7 - Upper Surface [MPa]"
      TWODELEMStressupper8 = "Stress Component 8 - Upper Surface [MPa]"

/* Coordinates */
      Points0 = "X-coordinate of node (mm)"
      Points1 = "Y-coordinate of node (mm)"
      Points2 = "Z-coordinate of node (mm)";
 ;

  input
     NODEID
     TWODELEMStrainlayer10
     TWODELEMStrainlayer11
     TWODELEMStrainlayer12
     TWODELEMStrainlayer13
     TWODELEMStrainlayer14
     TWODELEMStrainlayer15
     TWODELEMStrainlayer16
     TWODELEMStrainlayer17
     TWODELEMStrainlayer18
     TWODELEMStrainlayer20
     TWODELEMStrainlayer21
     TWODELEMStrainlayer22
     TWODELEMStrainlayer23
     TWODELEMStrainlayer24
     TWODELEMStrainlayer25
     TWODELEMStrainlayer26
     TWODELEMStrainlayer27
     TWODELEMStrainlayer28
     TWODELEMStrainlayer30
     TWODELEMStrainlayer31
     TWODELEMStrainlayer32
     TWODELEMStrainlayer33
     TWODELEMStrainlayer34
     TWODELEMStrainlayer35
     TWODELEMStrainlayer36
     TWODELEMStrainlayer37
     TWODELEMStrainlayer38
     TWODELEMStrainlayer40
     TWODELEMStrainlayer41
     TWODELEMStrainlayer42
     TWODELEMStrainlayer43
     TWODELEMStrainlayer44
     TWODELEMStrainlayer45
     TWODELEMStrainlayer46
     TWODELEMStrainlayer47
     TWODELEMStrainlayer48
     TWODELEMStrainlayer50
     TWODELEMStrainlayer51
     TWODELEMStrainlayer52
     TWODELEMStrainlayer53
     TWODELEMStrainlayer54
     TWODELEMStrainlayer55
     TWODELEMStrainlayer56
     TWODELEMStrainlayer57
     TWODELEMStrainlayer58
     TWODELEMStrainlower0
     TWODELEMStrainlower1
     TWODELEMStrainlower2
     TWODELEMStrainlower3
     TWODELEMStrainlower4
     TWODELEMStrainlower5
     TWODELEMStrainlower6
     TWODELEMStrainlower7
     TWODELEMStrainlower8
     TWODELEMStrainupper0
     TWODELEMStrainupper1
     TWODELEMStrainupper2
     TWODELEMStrainupper3
     TWODELEMStrainupper4
     TWODELEMStrainupper5
     TWODELEMStrainupper6
     TWODELEMStrainupper7
     TWODELEMStrainupper8
     TWODELEMStresslayer10
     TWODELEMStresslayer11
     TWODELEMStresslayer12
     TWODELEMStresslayer13
     TWODELEMStresslayer14
     TWODELEMStresslayer15
     TWODELEMStresslayer16
     TWODELEMStresslayer17
     TWODELEMStresslayer18
     TWODELEMStresslayer20
     TWODELEMStresslayer21
     TWODELEMStresslayer22
     TWODELEMStresslayer23
     TWODELEMStresslayer24
     TWODELEMStresslayer25
     TWODELEMStresslayer26
     TWODELEMStresslayer27
     TWODELEMStresslayer28
     TWODELEMStresslayer30
     TWODELEMStresslayer31
     TWODELEMStresslayer32
     TWODELEMStresslayer33
     TWODELEMStresslayer34
     TWODELEMStresslayer35
     TWODELEMStresslayer36
     TWODELEMStresslayer37
     TWODELEMStresslayer38
     TWODELEMStresslayer40
     TWODELEMStresslayer41
     TWODELEMStresslayer42
     TWODELEMStresslayer43
     TWODELEMStresslayer44
     TWODELEMStresslayer45
     TWODELEMStresslayer46
     TWODELEMStresslayer47
     TWODELEMStresslayer48
     TWODELEMStresslayer50
     TWODELEMStresslayer51
     TWODELEMStresslayer52
     TWODELEMStresslayer53
     TWODELEMStresslayer54
     TWODELEMStresslayer55
     TWODELEMStresslayer56
     TWODELEMStresslayer57
     TWODELEMStresslayer58
     TWODELEMStresslower0
     TWODELEMStresslower1
     TWODELEMStresslower2
     TWODELEMStresslower3
     TWODELEMStresslower4
     TWODELEMStresslower5
     TWODELEMStresslower6
     TWODELEMStresslower7
     TWODELEMStresslower8
     TWODELEMStressupper0
     TWODELEMStressupper1
     TWODELEMStressupper2
     TWODELEMStressupper3
     TWODELEMStressupper4
     TWODELEMStressupper5
     TWODELEMStressupper6
     TWODELEMStressupper7
     TWODELEMStressupper8
     ELEMENTID
     EROSIONSTATUS
     PARTID
     Points0
...
/**************************************************************************************************************************/
/*  Middle Observation(12033 ) of table = workx.stress_strain_tensors - Total Obs 24067                                   */
/*                                                                                                                        */
/*   -- NUMERIC --                                                                                                        */
/*  NODEID                           N8             407      Node Identification Number                                   */
/*  ELEMENTID                        N8           465.5      Element Identification Number                                */
/*  PARTID                           N8         2000001      Part Identification Number                                   */
/*  EROSIONSTATUS                    N8               1      Element Erosion Status (0=active, 1=eroded)                  */
/*  TWODELEMSTRAINLAYER10            N8    -0.007555382      Strain Tensor Component XX - Layer 1 (Bottom)                */
/*  TWODELEMSTRAINLAYER11            N8    -3.602251E-6      Strain Tensor Component YY - Layer 1 (Bottom)                */
/*  TWODELEMSTRAINLAYER12            N8               0      Strain Tensor Component ZZ - Layer 1 (Bottom)                */
/*  TWODELEMSTRAINLAYER13            N8    -3.602251E-6      Strain Tensor Component XY - Layer 1 (Bottom)                */
/*  TWODELEMSTRAINLAYER14            N8    0.0189679004      Strain Tensor Component YZ - Layer 1 (Bottom)                */
/*  TWODELEMSTRAINLAYER15            N8               0      Strain Tensor Component ZX - Layer 1 (Bottom)                */
/*  TWODELEMSTRAINLAYER16            N8               0      Strain Component 6 - Layer 1 (Bottom)                        */
/*  TWODELEMSTRAINLAYER17            N8               0      Strain Component 7 - Layer 1 (Bottom)                        */
/*  TWODELEMSTRAINLAYER18            N8               0      Strain Component 8 - Layer 1 (Bottom)                        */
/*  TWODELEMSTRAINLAYER20            N8    -0.007555382      Strain Tensor Component XX - Layer 2                         */
/*  TWODELEMSTRAINLAYER21            N8    -3.602251E-6      Strain Tensor Component YY - Layer 2                         */
/*  TWODELEMSTRAINLAYER22            N8               0      Strain Tensor Component ZZ - Layer 2                         */
/*  TWODELEMSTRAINLAYER23            N8    -3.602251E-6      Strain Tensor Component XY - Layer 2                         */
/*  TWODELEMSTRAINLAYER24            N8    0.0189679004      Strain Tensor Component YZ - Layer 2                         */
/*  TWODELEMSTRAINLAYER25            N8               0      Strain Tensor Component ZX - Layer 2                         */
/*  TWODELEMSTRAINLAYER26            N8               0      Strain Component 6 - Layer 2                                 */
/*  TWODELEMSTRAINLAYER27            N8               0      Strain Component 7 - Layer 2                                 */
/*  TWODELEMSTRAINLAYER28            N8               0      Strain Component 8 - Layer 2                                 */
/*  TWODELEMSTRAINLAYER30            N8    -0.007555382      Strain Tensor Component XX - Layer 3 (Middle)                */
/*  TWODELEMSTRAINLAYER31            N8    -3.602251E-6      Strain Tensor Component YY - Layer 3 (Middle)                */
/*  TWODELEMSTRAINLAYER32            N8               0      Strain Tensor Component ZZ - Layer 3 (Middle)                */
/*  TWODELEMSTRAINLAYER33            N8    -3.602251E-6      Strain Tensor Component XY - Layer 3 (Middle)                */
/*  TWODELEMSTRAINLAYER34            N8    0.0189679004      Strain Tensor Component YZ - Layer 3 (Middle)                */
/*  TWODELEMSTRAINLAYER35            N8               0      Strain Tensor Component ZX - Layer 3 (Middle)                */
/*  TWODELEMSTRAINLAYER36            N8               0      Strain Component 6 - Layer 3 (Middle)                        */
/*  TWODELEMSTRAINLAYER37            N8               0      Strain Component 7 - Layer 3 (Middle)                        */
/*  TWODELEMSTRAINLAYER38            N8               0      Strain Component 8 - Layer 3 (Middle)                        */
/*  TWODELEMSTRAINLAYER40            N8    -0.007555382      Strain Tensor Component XX - Layer 4                         */
/*  TWODELEMSTRAINLAYER41            N8    -3.602251E-6      Strain Tensor Component YY - Layer 4                         */
/*  TWODELEMSTRAINLAYER42            N8               0      Strain Tensor Component ZZ - Layer 4                         */
/*  TWODELEMSTRAINLAYER43            N8    -3.602251E-6      Strain Tensor Component XY - Layer 4                         */
/*  TWODELEMSTRAINLAYER44            N8    0.0189679004      Strain Tensor Component YZ - Layer 4                         */
/*  TWODELEMSTRAINLAYER45            N8               0      Strain Tensor Component ZX - Layer 4                         */
/*  TWODELEMSTRAINLAYER46            N8               0      Strain Component 6 - Layer 4                                 */
/*  TWODELEMSTRAINLAYER47            N8               0      Strain Component 7 - Layer 4                                 */
/*  TWODELEMSTRAINLAYER48            N8               0      Strain Component 8 - Layer 4                                 */
/*  TWODELEMSTRAINLAYER50            N8    -0.007555382      Strain Tensor Component XX - Layer 5 (Top)                   */
/*  TWODELEMSTRAINLAYER51            N8    -3.602251E-6      Strain Tensor Component YY - Layer 5 (Top)                   */
/*  TWODELEMSTRAINLAYER52            N8               0      Strain Tensor Component ZZ - Layer 5 (Top)                   */
/*  TWODELEMSTRAINLAYER53            N8    -3.602251E-6      Strain Tensor Component XY - Layer 5 (Top)                   */
/*  TWODELEMSTRAINLAYER54            N8    0.0189679004      Strain Tensor Component YZ - Layer 5 (Top)                   */
/*  TWODELEMSTRAINLAYER55            N8               0      Strain Tensor Component ZX - Layer 5 (Top)                   */
/*  TWODELEMSTRAINLAYER56            N8               0      Strain Component 6 - Layer 5 (Top)                           */
/*  TWODELEMSTRAINLAYER57            N8               0      Strain Component 7 - Layer 5 (Top)                           */
/*  TWODELEMSTRAINLAYER58            N8               0      Strain Component 8 - Layer 5 (Top)                           */
/*  TWODELEMSTRAINLOWER0             N8    -0.007555382      Strain Tensor Component XX - Lower Surface                   */
/*  TWODELEMSTRAINLOWER1             N8    -3.602251E-6      Strain Tensor Component YY - Lower Surface                   */
/*  TWODELEMSTRAINLOWER2             N8               0      Strain Tensor Component ZZ - Lower Surface                   */
/*  TWODELEMSTRAINLOWER3             N8    -3.602251E-6      Strain Tensor Component XY - Lower Surface                   */
/*  TWODELEMSTRAINLOWER4             N8    0.0189679004      Strain Tensor Component YZ - Lower Surface                   */
/*  TWODELEMSTRAINLOWER5             N8               0      Strain Tensor Component ZX - Lower Surface                   */
/*  TWODELEMSTRAINLOWER6             N8               0      Strain Component 6 - Lower Surface                           */
/*  TWODELEMSTRAINLOWER7             N8               0      Strain Component 7 - Lower Surface                           */
/*  TWODELEMSTRAINLOWER8             N8               0      Strain Component 8 - Lower Surface                           */
/*  TWODELEMSTRAINUPPER0             N8    -0.007555382      Strain Tensor Component XX - Upper Surface                   */
/*  TWODELEMSTRAINUPPER1             N8    -3.602251E-6      Strain Tensor Component YY - Upper Surface                   */
/*  TWODELEMSTRAINUPPER2             N8               0      Strain Tensor Component ZZ - Upper Surface                   */
/*  TWODELEMSTRAINUPPER3             N8    -3.602251E-6      Strain Tensor Component XY - Upper Surface                   */
/*  TWODELEMSTRAINUPPER4             N8    0.0189679004      Strain Tensor Component YZ - Upper Surface                   */
/*  TWODELEMSTRAINUPPER5             N8               0      Strain Tensor Component ZX - Upper Surface                   */
/*  TWODELEMSTRAINUPPER6             N8               0      Strain Component 6 - Upper Surface                           */
/*  TWODELEMSTRAINUPPER7             N8               0      Strain Component 7 - Upper Surface                           */
/*  TWODELEMSTRAINUPPER8             N8               0      Strain Component 8 - Upper Surface                           */
/*  TWODELEMSTRESSLAYER10            N8    51.342674255      Stress Tensor Component XX - Layer 1 (Bottom) [MPa]          */
/*  TWODELEMSTRESSLAYER11            N8    -2.394730091      Stress Tensor Component YY - Layer 1 (Bottom) [MPa]          */
/*  TWODELEMSTRESSLAYER12            N8               0      Stress Tensor Component ZZ - Layer 1 (Bottom) [MPa]          */
/*  TWODELEMSTRESSLAYER13            N8    -2.394730091      Stress Tensor Component XY - Layer 1 (Bottom) [MPa]          */
/*  TWODELEMSTRESSLAYER14            N8    506.00549316      Stress Tensor Component YZ - Layer 1 (Bottom) [MPa]          */
/*  TWODELEMSTRESSLAYER15            N8               0      Stress Tensor Component ZX - Layer 1 (Bottom) [MPa]          */
/*  TWODELEMSTRESSLAYER16            N8               0      Stress Component 6 - Layer 1 (Bottom) [MPa]                  */
/*  TWODELEMSTRESSLAYER17            N8               0      Stress Component 7 - Layer 1 (Bottom) [MPa]                  */
/*  TWODELEMSTRESSLAYER18            N8               0      Stress Component 8 - Layer 1 (Bottom) [MPa]                  */
/*  TWODELEMSTRESSLAYER20            N8    51.342674255      Stress Tensor Component XX - Layer 2 [MPa]                   */
/*  TWODELEMSTRESSLAYER21            N8    -2.394730091      Stress Tensor Component YY - Layer 2 [MPa]                   */
/*  TWODELEMSTRESSLAYER22            N8               0      Stress Tensor Component ZZ - Layer 2 [MPa]                   */
/*  TWODELEMSTRESSLAYER23            N8    -2.394730091      Stress Tensor Component XY - Layer 2 [MPa]                   */
/*  TWODELEMSTRESSLAYER24            N8    506.00549316      Stress Tensor Component YZ - Layer 2 [MPa]                   */
/*  TWODELEMSTRESSLAYER25            N8               0      Stress Tensor Component ZX - Layer 2 [MPa]                   */
/*  TWODELEMSTRESSLAYER26            N8               0      Stress Component 6 - Layer 2 [MPa]                           */
/*  TWODELEMSTRESSLAYER27            N8               0      Stress Component 7 - Layer 2 [MPa]                           */
/*  TWODELEMSTRESSLAYER28            N8               0      Stress Component 8 - Layer 2 [MPa]                           */
/*  TWODELEMSTRESSLAYER30            N8    51.342674255      Stress Tensor Component XX - Layer 3 (Middle) [MPa]          */
/*  TWODELEMSTRESSLAYER31            N8    -2.394730091      Stress Tensor Component YY - Layer 3 (Middle) [MPa]          */
/*  TWODELEMSTRESSLAYER32            N8               0      Stress Tensor Component ZZ - Layer 3 (Middle) [MPa]          */
/*  TWODELEMSTRESSLAYER33            N8    -2.394730091      Stress Tensor Component XY - Layer 3 (Middle) [MPa]          */
/*  TWODELEMSTRESSLAYER34            N8    506.00549316      Stress Tensor Component YZ - Layer 3 (Middle) [MPa]          */
/*  TWODELEMSTRESSLAYER35            N8               0      Stress Tensor Component ZX - Layer 3 (Middle) [MPa]          */
/*  TWODELEMSTRESSLAYER36            N8               0      Stress Component 6 - Layer 3 (Middle) [MPa]                  */
/*  TWODELEMSTRESSLAYER37            N8               0      Stress Component 7 - Layer 3 (Middle) [MPa]                  */
/*  TWODELEMSTRESSLAYER38            N8               0      Stress Component 8 - Layer 3 (Middle) [MPa]                  */
/*  TWODELEMSTRESSLAYER40            N8    51.342674255      Stress Tensor Component XX - Layer 4 [MPa]                   */
/*  TWODELEMSTRESSLAYER41            N8    -2.394730091      Stress Tensor Component YY - Layer 4 [MPa]                   */
/*  TWODELEMSTRESSLAYER42            N8               0      Stress Tensor Component ZZ - Layer 4 [MPa]                   */
/*  TWODELEMSTRESSLAYER43            N8    -2.394730091      Stress Tensor Component XY - Layer 4 [MPa]                   */
/*  TWODELEMSTRESSLAYER44            N8    506.00549316      Stress Tensor Component YZ - Layer 4 [MPa]                   */
/*  TWODELEMSTRESSLAYER45            N8               0      Stress Tensor Component ZX - Layer 4 [MPa]                   */
/*  TWODELEMSTRESSLAYER46            N8               0      Stress Component 6 - Layer 4 [MPa]                           */
/*  TWODELEMSTRESSLAYER47            N8               0      Stress Component 7 - Layer 4 [MPa]                           */
/*  TWODELEMSTRESSLAYER48            N8               0      Stress Component 8 - Layer 4 [MPa]                           */
/*  TWODELEMSTRESSLAYER50            N8    51.342674255      Stress Tensor Component XX - Layer 5 (Top) [MPa]             */
/*  TWODELEMSTRESSLAYER51            N8    -2.394730091      Stress Tensor Component YY - Layer 5 (Top) [MPa]             */
/*  TWODELEMSTRESSLAYER52            N8               0      Stress Tensor Component ZZ - Layer 5 (Top) [MPa]             */
/*  TWODELEMSTRESSLAYER53            N8    -2.394730091      Stress Tensor Component XY - Layer 5 (Top) [MPa]             */
/*  TWODELEMSTRESSLAYER54            N8    506.00549316      Stress Tensor Component YZ - Layer 5 (Top) [MPa]             */
/*  TWODELEMSTRESSLAYER55            N8               0      Stress Tensor Component ZX - Layer 5 (Top) [MPa]             */
/*  TWODELEMSTRESSLAYER56            N8               0      Stress Component 6 - Layer 5 (Top) [MPa]                     */
/*  TWODELEMSTRESSLAYER57            N8               0      Stress Component 7 - Layer 5 (Top) [MPa]                     */
/*  TWODELEMSTRESSLAYER58            N8               0      Stress Component 8 - Layer 5 (Top) [MPa]                     */
/*  TWODELEMSTRESSLOWER0             N8    51.342674255      Stress Tensor Component XX - Lower Surface [MPa]             */
/*  TWODELEMSTRESSLOWER1             N8    -2.394730091      Stress Tensor Component YY - Lower Surface [MPa]             */
/*  TWODELEMSTRESSLOWER2             N8               0      Stress Tensor Component ZZ - Lower Surface [MPa]             */
/*  TWODELEMSTRESSLOWER3             N8    -2.394730091      Stress Tensor Component XY - Lower Surface [MPa]             */
/*  TWODELEMSTRESSLOWER4             N8    506.00549316      Stress Tensor Component YZ - Lower Surface [MPa]             */
/*  TWODELEMSTRESSLOWER5             N8               0      Stress Tensor Component ZX - Lower Surface [MPa]             */
/*  TWODELEMSTRESSLOWER6             N8               0      Stress Component 6 - Lower Surface [MPa]                     */
/*  TWODELEMSTRESSLOWER7             N8               0      Stress Component 7 - Lower Surface [MPa]                     */
/*  TWODELEMSTRESSLOWER8             N8               0      Stress Component 8 - Lower Surface [MPa]                     */
/*  TWODELEMSTRESSUPPER0             N8    51.342674255      Stress Tensor Component XX - Upper Surface [MPa]             */
/*  TWODELEMSTRESSUPPER1             N8    -2.394730091      Stress Tensor Component YY - Upper Surface [MPa]             */
/*  TWODELEMSTRESSUPPER2             N8               0      Stress Tensor Component ZZ - Upper Surface [MPa]             */
/*  TWODELEMSTRESSUPPER3             N8    -2.394730091      Stress Tensor Component XY - Upper Surface [MPa]             */
/*  TWODELEMSTRESSUPPER4             N8    506.00549316      Stress Tensor Component YZ - Upper Surface [MPa]             */
/*  TWODELEMSTRESSUPPER5             N8               0      Stress Tensor Component ZX - Upper Surface [MPa]             */
/*  TWODELEMSTRESSUPPER6             N8               0      Stress Component 6 - Upper Surface [MPa]                     */
/*  TWODELEMSTRESSUPPER7             N8               0      Stress Component 7 - Upper Surface [MPa]                     */
/*  TWODELEMSTRESSUPPER8             N8               0      Stress Component 8 - Upper Surface [MPa]                     */
/*  POINTS0                          N8    186.61099243      X-coordinate of node (mm)                                    */
/*  POINTS1                          N8    1.7981699705      Y-coordinate of node (mm)                                    */
/*  POINTS2                          N8     2.69374E-16      Z-coordinate of node (mm)                                    */
/**************************************************************************************************************************/
...
1                                          Altair SLC         14:58 Wednesday, May 20, 2026
...
1          options ls=255;
2
3         data workx.stress_strain_tensors;
4
5           infile "d:/rad/converted_stress_strain.csv" delimiter=',' dsd missover lrecl=32756 firstobs=2;
6
7         /* Element identification */
8         label NODEID       = "Node Identification Number"
9               ELEMENTID    = "Element Identification Number"
10              PARTID       = "Part Identification Number"
11              EROSIONSTATUS = "Element Erosion Status (0=active, 1=eroded)"
12
13        /* ----- STRAIN TENSOR COMPONENTS (Layer 1-5) ----- */
14        /* Layer 1 (Bottom integration point) */
15            l TWODELEMStrainlayer10 = "Strain Tensor Component XX - Layer 1 (Bottom)"
16              TWODELEMStrainlayer11 = "Strain Tensor Component YY - Layer 1 (Bottom)"
17              TWODELEMStrainlayer12 = "Strain Tensor Component ZZ - Layer 1 (Bottom)"
18              TWODELEMStrainlayer13 = "Strain Tensor Component XY - Layer 1 (Bottom)"
19              TWODELEMStrainlayer14 = "Strain Tensor Component YZ - Layer 1 (Bottom)"
20              TWODELEMStrainlayer15 = "Strain Tensor Component ZX - Layer 1 (Bottom)"
21              TWODELEMStrainlayer16 = "Strain Component 6 - Layer 1 (Bottom)"
22              TWODELEMStrainlayer17 = "Strain Component 7 - Layer 1 (Bottom)"
23              TWODELEMStrainlayer18 = "Strain Component 8 - Layer 1 (Bottom)"
24
25        /* Layer 2 (Second integration point) */
26              TWODELEMStrainlayer20 = "Strain Tensor Component XX - Layer 2"
27              TWODELEMStrainlayer21 = "Strain Tensor Component YY - Layer 2"
28              TWODELEMStrainlayer22 = "Strain Tensor Component ZZ - Layer 2"
29              TWODELEMStrainlayer23 = "Strain Tensor Component XY - Layer 2"
30              TWODELEMStrainlayer24 = "Strain Tensor Component YZ - Layer 2"
31              TWODELEMStrainlayer25 = "Strain Tensor Component ZX - Layer 2"
32              TWODELEMStrainlayer26 = "Strain Component 6 - Layer 2"
33              TWODELEMStrainlayer27 = "Strain Component 7 - Layer 2"
34              TWODELEMStrainlayer28 = "Strain Component 8 - Layer 2"
35
36        /* Layer 3 (Middle integration point) */
37              TWODELEMStrainlayer30 = "Strain Tensor Component XX - Layer 3 (Middle)"
38              TWODELEMStrainlayer31 = "Strain Tensor Component YY - Layer 3 (Middle)"
39              TWODELEMStrainlayer32 = "Strain Tensor Component ZZ - Layer 3 (Middle)"
40              TWODELEMStrainlayer33 = "Strain Tensor Component XY - Layer 3 (Middle)"
41              TWODELEMStrainlayer34 = "Strain Tensor Component YZ - Layer 3 (Middle)"
42              TWODELEMStrainlayer35 = "Strain Tensor Component ZX - Layer 3 (Middle)"
43              TWODELEMStrainlayer36 = "Strain Component 6 - Layer 3 (Middle)"
44              TWODELEMStrainlayer37 = "Strain Component 7 - Layer 3 (Middle)"
45              TWODELEMStrainlayer38 = "Strain Component 8 - Layer 3 (Middle)"
46
47        /* Layer 4 (Fourth integration point) */
48              TWODELEMStrainlayer40 = "Strain Tensor Component XX - Layer 4"
49              TWODELEMStrainlayer41 = "Strain Tensor Component YY - Layer 4"
50              TWODELEMStrainlayer42 = "Strain Tensor Component ZZ - Layer 4"
51              TWODELEMStrainlayer43 = "Strain Tensor Component XY - Layer 4"
52              TWODELEMStrainlayer44 = "Strain Tensor Component YZ - Layer 4"
53              TWODELEMStrainlayer45 = "Strain Tensor Component ZX - Layer 4"
54              TWODELEMStrainlayer46 = "Strain Component 6 - Layer 4"
55              TWODELEMStrainlayer47 = "Strain Component 7 - Layer 4"
56              TWODELEMStrainlayer48 = "Strain Component 8 - Layer 4"
57
58        /* Layer 5 (Top integration point) */
59              TWODELEMStrainlayer50 = "Strain Tensor Component XX - Layer 5 (Top)"
60              TWODELEMStrainlayer51 = "Strain Tensor Component YY - Layer 5 (Top)"
61              TWODELEMStrainlayer52 = "Strain Tensor Component ZZ - Layer 5 (Top)"
62              TWODELEMStrainlayer53 = "Strain Tensor Component XY - Layer 5 (Top)"
63              TWODELEMStrainlayer54 = "Strain Tensor Component YZ - Layer 5 (Top)"
64              TWODELEMStrainlayer55 = "Strain Tensor Component ZX - Layer 5 (Top)"
65              TWODELEMStrainlayer56 = "Strain Component 6 - Layer 5 (Top)"
66              TWODELEMStrainlayer57 = "Strain Component 7 - Layer 5 (Top)"
67              TWODELEMStrainlayer58 = "Strain Component 8 - Layer 5 (Top)"
68
69        /* Lower surface strain (extrapolated) */
70              TWODELEMStrainlower0 = "Strain Tensor Component XX - Lower Surface"
71              TWODELEMStrainlower1 = "Strain Tensor Component YY - Lower Surface"
72              TWODELEMStrainlower2 = "Strain Tensor Component ZZ - Lower Surface"
73              TWODELEMStrainlower3 = "Strain Tensor Component XY - Lower Surface"
74              TWODELEMStrainlower4 = "Strain Tensor Component YZ - Lower Surface"
75              TWODELEMStrainlower5 = "Strain Tensor Component ZX - Lower Surface"
76              TWODELEMStrainlower6 = "Strain Component 6 - Lower Surface"
77              TWODELEMStrainlower7 = "Strain Component 7 - Lower Surface"
78              TWODELEMStrainlower8 = "Strain Component 8 - Lower Surface"
79
80        /* Upper surface strain (extrapolated) */
81              TWODELEMStrainupper0 = "Strain Tensor Component XX - Upper Surface"
82              TWODELEMStrainupper1 = "Strain Tensor Component YY - Upper Surface"
83              TWODELEMStrainupper2 = "Strain Tensor Component ZZ - Upper Surface"
84              TWODELEMStrainupper3 = "Strain Tensor Component XY - Upper Surface"
85              TWODELEMStrainupper4 = "Strain Tensor Component YZ - Upper Surface"
86              TWODELEMStrainupper5 = "Strain Tensor Component ZX - Upper Surface"
87              TWODELEMStrainupper6 = "Strain Component 6 - Upper Surface"
88              TWODELEMStrainupper7 = "Strain Component 7 - Upper Surface"
89              TWODELEMStrainupper8 = "Strain Component 8 - Upper Surface"
90
91        /* ----- STRESS TENSOR COMPONENTS (Layer 1-5) ----- */
92        /* Layer 1 (Bottom) Stress */
93              TWODELEMStresslayer10 = "Stress Tensor Component XX - Layer 1 (Bottom) [MPa]"
94              TWODELEMStresslayer11 = "Stress Tensor Component YY - Layer 1 (Bottom) [MPa]"
95              TWODELEMStresslayer12 = "Stress Tensor Component ZZ - Layer 1 (Bottom) [MPa]"
96              TWODELEMStresslayer13 = "Stress Tensor Component XY - Layer 1 (Bottom) [MPa]"
97              TWODELEMStresslayer14 = "Stress Tensor Component YZ - Layer 1 (Bottom) [MPa]"
98              TWODELEMStresslayer15 = "Stress Tensor Component ZX - Layer 1 (Bottom) [MPa]"
99              TWODELEMStresslayer16 = "Stress Component 6 - Layer 1 (Bottom) [MPa]"
100             TWODELEMStresslayer17 = "Stress Component 7 - Layer 1 (Bottom) [MPa]"
101             TWODELEMStresslayer18 = "Stress Component 8 - Layer 1 (Bottom) [MPa]"
102
103       /* Layer 2 Stress */
104             TWODELEMStresslayer20 = "Stress Tensor Component XX - Layer 2 [MPa]"
105             TWODELEMStresslayer21 = "Stress Tensor Component YY - Layer 2 [MPa]"
106             TWODELEMStresslayer22 = "Stress Tensor Component ZZ - Layer 2 [MPa]"
107             TWODELEMStresslayer23 = "Stress Tensor Component XY - Layer 2 [MPa]"
108             TWODELEMStresslayer24 = "Stress Tensor Component YZ - Layer 2 [MPa]"
109             TWODELEMStresslayer25 = "Stress Tensor Component ZX - Layer 2 [MPa]"
110             TWODELEMStresslayer26 = "Stress Component 6 - Layer 2 [MPa]"
111             TWODELEMStresslayer27 = "Stress Component 7 - Layer 2 [MPa]"
112             TWODELEMStresslayer28 = "Stress Component 8 - Layer 2 [MPa]"
113
114       /* Layer 3 (Middle) Stress */
115             TWODELEMStresslayer30 = "Stress Tensor Component XX - Layer 3 (Middle) [MPa]"
116             TWODELEMStresslayer31 = "Stress Tensor Component YY - Layer 3 (Middle) [MPa]"
117             TWODELEMStresslayer32 = "Stress Tensor Component ZZ - Layer 3 (Middle) [MPa]"
118             TWODELEMStresslayer33 = "Stress Tensor Component XY - Layer 3 (Middle) [MPa]"
119             TWODELEMStresslayer34 = "Stress Tensor Component YZ - Layer 3 (Middle) [MPa]"
120             TWODELEMStresslayer35 = "Stress Tensor Component ZX - Layer 3 (Middle) [MPa]"
121             TWODELEMStresslayer36 = "Stress Component 6 - Layer 3 (Middle) [MPa]"
122             TWODELEMStresslayer37 = "Stress Component 7 - Layer 3 (Middle) [MPa]"
123             TWODELEMStresslayer38 = "Stress Component 8 - Layer 3 (Middle) [MPa]"
124
125       /* Layer 4 Stress */
126             TWODELEMStresslayer40 = "Stress Tensor Component XX - Layer 4 [MPa]"
127             TWODELEMStresslayer41 = "Stress Tensor Component YY - Layer 4 [MPa]"
128             TWODELEMStresslayer42 = "Stress Tensor Component ZZ - Layer 4 [MPa]"
129             TWODELEMStresslayer43 = "Stress Tensor Component XY - Layer 4 [MPa]"
130             TWODELEMStresslayer44 = "Stress Tensor Component YZ - Layer 4 [MPa]"
131             TWODELEMStresslayer45 = "Stress Tensor Component ZX - Layer 4 [MPa]"
132             TWODELEMStresslayer46 = "Stress Component 6 - Layer 4 [MPa]"
133             TWODELEMStresslayer47 = "Stress Component 7 - Layer 4 [MPa]"
134             TWODELEMStresslayer48 = "Stress Component 8 - Layer 4 [MPa]"
135
136       /* Layer 5 (Top) Stress */
137             TWODELEMStresslayer50 = "Stress Tensor Component XX - Layer 5 (Top) [MPa]"
138             TWODELEMStresslayer51 = "Stress Tensor Component YY - Layer 5 (Top) [MPa]"
139             TWODELEMStresslayer52 = "Stress Tensor Component ZZ - Layer 5 (Top) [MPa]"
140             TWODELEMStresslayer53 = "Stress Tensor Component XY - Layer 5 (Top) [MPa]"
141             TWODELEMStresslayer54 = "Stress Tensor Component YZ - Layer 5 (Top) [MPa]"
142             TWODELEMStresslayer55 = "Stress Tensor Component ZX - Layer 5 (Top) [MPa]"
143             TWODELEMStresslayer56 = "Stress Component 6 - Layer 5 (Top) [MPa]"
144             TWODELEMStresslayer57 = "Stress Component 7 - Layer 5 (Top) [MPa]"
145             TWODELEMStresslayer58 = "Stress Component 8 - Layer 5 (Top) [MPa]"
146
147       /* Lower surface stress (extrapolated) */
148             TWODELEMStresslower0 = "Stress Tensor Component XX - Lower Surface [MPa]"
149             TWODELEMStresslower1 = "Stress Tensor Component YY - Lower Surface [MPa]"
150             TWODELEMStresslower2 = "Stress Tensor Component ZZ - Lower Surface [MPa]"
151             TWODELEMStresslower3 = "Stress Tensor Component XY - Lower Surface [MPa]"
152             TWODELEMStresslower4 = "Stress Tensor Component YZ - Lower Surface [MPa]"
153             TWODELEMStresslower5 = "Stress Tensor Component ZX - Lower Surface [MPa]"
154             TWODELEMStresslower6 = "Stress Component 6 - Lower Surface [MPa]"
155             TWODELEMStresslower7 = "Stress Component 7 - Lower Surface [MPa]"
156             TWODELEMStresslower8 = "Stress Component 8 - Lower Surface [MPa]"
157
158       /* Upper surface stress (extrapolated) */
159             TWODELEMStressupper0 = "Stress Tensor Component XX - Upper Surface [MPa]"
160             TWODELEMStressupper1 = "Stress Tensor Component YY - Upper Surface [MPa]"
161             TWODELEMStressupper2 = "Stress Tensor Component ZZ - Upper Surface [MPa]"
162             TWODELEMStressupper3 = "Stress Tensor Component XY - Upper Surface [MPa]"
163             TWODELEMStressupper4 = "Stress Tensor Component YZ - Upper Surface [MPa]"
164             TWODELEMStressupper5 = "Stress Tensor Component ZX - Upper Surface [MPa]"
165             TWODELEMStressupper6 = "Stress Component 6 - Upper Surface [MPa]"
166             TWODELEMStressupper7 = "Stress Component 7 - Upper Surface [MPa]"
167             TWODELEMStressupper8 = "Stress Component 8 - Upper Surface [MPa]"
168
169       /* Coordinates */
170             Points0 = "X-coordinate of node (mm)"
171             Points1 = "Y-coordinate of node (mm)"
172             Points2 = "Z-coordinate of node (mm)";
173        ;
174
175         input
176            NODEID
177            TWODELEMStrainlayer10
178            TWODELEMStrainlayer11
179            TWODELEMStrainlayer12
180            TWODELEMStrainlayer13
181            TWODELEMStrainlayer14
182            TWODELEMStrainlayer15
183            TWODELEMStrainlayer16
184            TWODELEMStrainlayer17
185            TWODELEMStrainlayer18
186            TWODELEMStrainlayer20
187            TWODELEMStrainlayer21
188            TWODELEMStrainlayer22
189            TWODELEMStrainlayer23
190            TWODELEMStrainlayer24
191            TWODELEMStrainlayer25
192            TWODELEMStrainlayer26
193            TWODELEMStrainlayer27
194            TWODELEMStrainlayer28
195            TWODELEMStrainlayer30
196            TWODELEMStrainlayer31
197            TWODELEMStrainlayer32
198            TWODELEMStrainlayer33
199            TWODELEMStrainlayer34
200            TWODELEMStrainlayer35
201            TWODELEMStrainlayer36
202            TWODELEMStrainlayer37
203            TWODELEMStrainlayer38
204            TWODELEMStrainlayer40
205            TWODELEMStrainlayer41
206            TWODELEMStrainlayer42
207            TWODELEMStrainlayer43
208            TWODELEMStrainlayer44
209            TWODELEMStrainlayer45
210            TWODELEMStrainlayer46
211            TWODELEMStrainlayer47
212            TWODELEMStrainlayer48
213            TWODELEMStrainlayer50
214            TWODELEMStrainlayer51
215            TWODELEMStrainlayer52
216            TWODELEMStrainlayer53
217            TWODELEMStrainlayer54
218            TWODELEMStrainlayer55
219            TWODELEMStrainlayer56
220            TWODELEMStrainlayer57
221            TWODELEMStrainlayer58
222            TWODELEMStrainlower0
223            TWODELEMStrainlower1
224            TWODELEMStrainlower2
225            TWODELEMStrainlower3
226            TWODELEMStrainlower4
227            TWODELEMStrainlower5
228            TWODELEMStrainlower6
229            TWODELEMStrainlower7
230            TWODELEMStrainlower8
231            TWODELEMStrainupper0
232            TWODELEMStrainupper1
233            TWODELEMStrainupper2
234            TWODELEMStrainupper3
235            TWODELEMStrainupper4
236            TWODELEMStrainupper5
237            TWODELEMStrainupper6
238            TWODELEMStrainupper7
239            TWODELEMStrainupper8
240            TWODELEMStresslayer10
241            TWODELEMStresslayer11
242            TWODELEMStresslayer12
243            TWODELEMStresslayer13
244            TWODELEMStresslayer14
245            TWODELEMStresslayer15
246            TWODELEMStresslayer16
247            TWODELEMStresslayer17
248            TWODELEMStresslayer18
249            TWODELEMStresslayer20
250            TWODELEMStresslayer21
251            TWODELEMStresslayer22
252            TWODELEMStresslayer23
253            TWODELEMStresslayer24
254            TWODELEMStresslayer25
255            TWODELEMStresslayer26
256            TWODELEMStresslayer27
257            TWODELEMStresslayer28
258            TWODELEMStresslayer30
259            TWODELEMStresslayer31
260            TWODELEMStresslayer32
261            TWODELEMStresslayer33
262            TWODELEMStresslayer34
263            TWODELEMStresslayer35
264            TWODELEMStresslayer36
265            TWODELEMStresslayer37
266            TWODELEMStresslayer38
267            TWODELEMStresslayer40
268            TWODELEMStresslayer41
269            TWODELEMStresslayer42
270            TWODELEMStresslayer43
271            TWODELEMStresslayer44
272            TWODELEMStresslayer45
273            TWODELEMStresslayer46
274            TWODELEMStresslayer47
275            TWODELEMStresslayer48
276            TWODELEMStresslayer50
277            TWODELEMStresslayer51
278            TWODELEMStresslayer52
279            TWODELEMStresslayer53
280            TWODELEMStresslayer54
281            TWODELEMStresslayer55
282            TWODELEMStresslayer56
283            TWODELEMStresslayer57
284            TWODELEMStresslayer58
285            TWODELEMStresslower0
286            TWODELEMStresslower1
287            TWODELEMStresslower2
288            TWODELEMStresslower3
289            TWODELEMStresslower4
290            TWODELEMStresslower5
291            TWODELEMStresslower6
292            TWODELEMStresslower7
293            TWODELEMStresslower8
294            TWODELEMStressupper0
295            TWODELEMStressupper1
296            TWODELEMStressupper2
297            TWODELEMStressupper3
298            TWODELEMStressupper4
299            TWODELEMStressupper5
300            TWODELEMStressupper6
301            TWODELEMStressupper7
302            TWODELEMStressupper8
303            ELEMENTID
304            EROSIONSTATUS
305            PARTID
306            Points0
...
NOTE: The infile 'd:\rad\converted_stress_strain.csv' is:
      Filename='d:\rad\converted_stress_strain.csv',
      Owner Name=SLC\suzie,
      File size (bytes)=23664620,
      Create Time=14:31:17 May 19 2026,
      Last Accessed=13:52:09 Jun 01 2022,
      Last Modified=14:31:23 May 19 2026,
      Lrecl=32756, Recfm=V

NOTE: 24067 records were read from file 'd:\rad\converted_stress_strain.csv'
      The minimum record length was 277
      The maximum record length was 1180
NOTE: Data set "WORKX.stress_strain_tensors" has 24067 observation(s) and 133 variable(s)
NOTE: The data step took :
      real time : 0.681
      cpu time  : 0.687
...
/ |___ /  __   _| |_| | ____| |/ _| | |_ ___    _ __ ___  _ __ | || |
| | |_ \  \ \ / / __| |/ / _` | |_  | __/ _ \  | `_ ` _ \| `_ \| || |_
| |___) |  \ V /| |_|   < (_| |  _| | || (_) | | | | | | | |_) |__   _|
|_|____/    \_/  \__|_|\_\__,_|_|    \__\___/  |_| |_| |_| .__/   |_|
                                                         |_|
*/
...
#!/usr/bin/env python
"""
Convert VTKHDF to MP4 with stress contours for tensile test.
Run with: pvpython this_script.py
"""
...
# ============================================================
VTKHDF_FILE = "D:/rad/gmsh_tensile_LAW36_BIQUAD.vtkhdf"
OUTPUT_DIR = "D:/rad/enhancedframes"
OUTPUT_VIDEO = "D:/rad/enhanced_animation.mp4"
FRAME_RATE = 30
...
# Use upper surface stress (cell data) - convert to point data first
STRESS_ARRAY = "2DELEM_Stress_(upper)"
...
print("=" * 60)
print("VTKHDF to MP4 Converter with Stress Contours")
print("=" * 60)
...
# ============================================================
print(f"\nConverting cell data to point data using '{STRESS_ARRAY}'...")
cell_to_point = CellDatatoPointData(Input=source)
cell_to_point.UpdatePipeline()

# ============================================================
# STEP 2: Create Render View and Apply Coloring
# ============================================================
...
# Apply coloring using the converted point data
print(f"\nApplying stress coloring using '{STRESS_ARRAY}'...")
display.ColorArrayName = ['POINTS', STRESS_ARRAY]
display.SetScalarBarVisibility(renderView, True)

# Configure color map
lut = GetColorTransferFunction(STRESS_ARRAY)
...
data_range = cell_to_point.PointData.GetArray(STRESS_ARRAY).GetRange()
...
/* d:/rad                                                                                                                 */
/*                                                                                                                        */
/*   D:/rad/gmsh_tensile_LAW36_BIQUAD.vtkhdf                                                                              */
/*   gmsh_tensile_LAW36_BIQUADA003.vtk   8,998 KB                                                                         */
/*                                                                                                                        */
/* OUTPUTS                                                                                                                */
/* --------------------------------------------                                                                           */
/* d:/rad/frame                                                                                                           */
...
1                                          Altair SLC         15:11 Wednesday, May 20, 2026
...
NOTE: The file 'c:\temp\py_pgmx.py' is:
      ...
      File size (bytes)=0,
      Create Time=13:21:25 Jan 12 2026,
      Last Accessed=15:11:03 May 20 2026,
      Last Modified=15:11:03 May 20 2026,
      Lrecl=32767, Recfm=V

NOTE: 197 records were written to file 'c:\temp\py_pgmx.py'
      The minimum record length was 80
      The maximum record length was 181
NOTE: The data step took :
      real time : 0.004
...
4         """
5         Convert VTKHDF to MP4 with stress contours for tensile test.
...
14        # CONFIGURATION
15        # ============================================================
16        VTKHDF_FILE = "D:/rad/gmsh_tensile_LAW36_BIQUAD.vtkhdf"
17        OUTPUT_DIR = "D:/rad/enhancedframes"
18        OUTPUT_VIDEO = "D:/rad/enhanced_animation.mp4"
19        FRAME_RATE = 30
20
21        IMAGE_WIDTH = 1920
22        IMAGE_HEIGHT = 1080
23
24        CAMERA_POSITION = [0, 0, 15]
25        CAMERA_FOCAL_POINT = [0, 0, 0]
26        CAMERA_VIEW_UP = [0, 1, 0]
27        USE_PARALLEL_PROJECTION = True
28        PARALLEL_SCALE = 5.0
29
30        # Use upper surface stress (cell data) - convert to point data first
31        STRESS_ARRAY = "2DELEM_Stress_(upper)"
32        COLOR_MAP = "Cool to Warm"
33
34        # ============================================================
...
38
39        print("=" * 60)
40        print("VTKHDF to MP4 Converter with Stress Contours")
41        print("=" * 60)
42        print(f"Input file: {VTKHDF_FILE}")
43        print(f"Output video: {OUTPUT_VIDEO}")
44        print(f"Frame rate: {FRAME_RATE} fps")
45
46        # ============================================================
47        # LOAD THE VTKHDF FILE
48        # ============================================================
49        if not os.path.exists(VTKHDF_FILE):
50            print(f"ERROR: File not found: {VTKHDF_FILE}")
51            exit(1)
52
53        print("\nLoading VTKHDF file...")
54        source = OpenDataFile(VTKHDF_FILE)
55        source.UpdatePipeline()
56
57        timesteps = source.TimestepValues
58        num_frames = len(timesteps) if timesteps else 1
59        print(f"Found {num_frames} time steps")
60
61        print(f"Point Data: {list(source.PointData.keys())}")
62        print(f"Cell Data: {list(source.CellData.keys())}")
63
64        # ============================================================
65        # STEP 1: Convert Cell Data to Point Data
66        # ============================================================
67        print(f"\nConverting cell data to point data using '{STRESS_ARRAY}'...")
68        cell_to_point = CellDatatoPointData(Input=source)
69        cell_to_point.UpdatePipeline()
70
71        # ============================================================
72        # STEP 2: Create Render View and Apply Coloring
73        # ============================================================
74        print("\nSetting up render view...")
75        renderView = CreateView('RenderView')
76        renderView.ViewSize = [IMAGE_WIDTH, IMAGE_HEIGHT]
77        AssignViewToLayout(renderView)
78
79        display = Show(cell_to_point, renderView)
80        display.Representation = 'Surface'
81
82        # Apply coloring using the converted point data
83        print(f"\nApplying stress coloring using '{STRESS_ARRAY}'...")
84        display.ColorArrayName = ['POINTS', STRESS_ARRAY]
85        display.SetScalarBarVisibility(renderView, True)
86
87        # Configure color map
88        lut = GetColorTransferFunction(STRESS_ARRAY)
89        if COLOR_MAP == "Cool to Warm":
90            lut.ColorSpace = 'Diverging'
91        elif COLOR_MAP == "Jet":
92            lut.ColorSpace = 'RGB'
93
94        # Get data range and rescale
95        data_range = cell_to_point.PointData.GetArray(STRESS_ARRAY).GetRange()
96        print(f"  Data range: {data_range[0]:.6f} to {data_range[1]:.6f}")
97        if data_range[1] > data_range[0]:
98            lut.RescaleTransferFunction(data_range[0], data_range[1])
99            print(f"  Color map '{COLOR_MAP}' applied")
100
101       display.Representation = 'Surface'
102       renderView.Background = [0.1, 0.2, 0.4]
103
104       # ============================================================
105       # STEP 3: Set Up Camera
106       # ============================================================
107       renderView.CameraPosition = CAMERA_POSITION
108       renderView.CameraFocalPoint = CAMERA_FOCAL_POINT
109       renderView.CameraViewUp = CAMERA_VIEW_UP
110
111       if USE_PARALLEL_PROJECTION:
112           renderView.CameraParallelProjection = 1
113           renderView.CameraParallelScale = PARALLEL_SCALE
114       else:
115           renderView.CameraParallelProjection = 0
116
117       renderView.ResetCamera()
118       Render()
119
120       # ============================================================
121       # STEP 4: Configure Animation
122       # ============================================================
123       animationScene = GetAnimationScene()
124       animationScene.NumberOfFrames = num_frames
125       animationScene.StartTime = 0
126       animationScene.EndTime = num_frames - 1
127       animationScene.PlayMode = 'Sequence'
128
129       # ============================================================
130       # STEP 5: Save Frames
131       # ============================================================
132       print(f"\nSaving {num_frames} frames to {OUTPUT_DIR}...")
133
134       for i in range(num_frames):
135           animationScene.TimeKeeper.Time = i
136           cell_to_point.UpdatePipeline(i)
137           Render()
138
139           frame_file = os.path.join(OUTPUT_DIR, f"frame_{i+1:04d}.png")
140           SaveScreenshot(frame_file, renderView, ImageResolution=[IMAGE_WIDTH, IMAGE_HEIGHT])
141
142           if (i + 1) % 10 == 0 or (i + 1) == num_frames:
143               print(f"  Saved frame {i+1}/{num_frames}")
144
145       print(f"\nFrames saved to: {OUTPUT_DIR}")
146
147       # ============================================================
148       # STEP 6: Convert to MP4
149       # ============================================================
150       print("\n" + "=" * 50)
151       print("Converting PNG sequence to MP4...")
152       print("=" * 50)
153
154       ffmpeg_cmd = None
155       ffmpeg_paths = ['ffmpeg', 'C:\\ffmpeg\\bin\\ffmpeg.exe', 'D:\\ffmpeg\\bin\\ffmpeg.exe']
156
157       for path in ffmpeg_paths:
158           try:
159               subprocess.run([path, '-version'], capture_output=True, check=True)
160               ffmpeg_cmd = path
161               break
162           except (subprocess.SubprocessError, FileNotFoundError):
163               continue
164
165       if ffmpeg_cmd:
166           input_pattern = os.path.join(OUTPUT_DIR, "frame_%04d.png").replace('\\', '/')
167           output_video = OUTPUT_VIDEO.replace('\\', '/')
168
169           ffmpeg_command = [
170               ffmpeg_cmd, '-framerate', str(FRAME_RATE),
171               '-i', input_pattern,
172               '-vf', 'pad=1920:1062:(ow-iw)/2:(oh-ih)/2',
173               '-c:v', 'libx264',
174               '-pix_fmt', 'yuv420p',
175               '-crf', '18',
176               '-y', output_video
177           ]
178
179           print(f"Running FFmpeg...")
180           try:
181               result = subprocess.run(ffmpeg_command, capture_output=True, text=True)
182               if result.returncode == 0 and os.path.exists(OUTPUT_VIDEO):
183                   size_mb = os.path.getsize(OUTPUT_VIDEO) / (1024 * 1024)
184                   print(f"\nSUCCESS! MP4 created: {OUTPUT_VIDEO}")
185                   print(f"File size: {size_mb:.2f} MB")
186               else:
187                   print(f"\nFFmpeg failed. Run this command manually:")
188                   print(f'ffmpeg -framerate {FRAME_RATE} -i "{OUTPUT_DIR}/frame_%04d.png" -vf "pad=1920:1062:(ow-iw)/2:(oh-ih)/2" -c:v libx264 -pix_fmt yuv420p -crf 18 -y {OUTPUT_VIDEO}')
189           except Exception as e:
190               print(f"\nError: {e}")
191               print(f'ffmpeg -framerate {FRAME_RATE} -i "{OUTPUT_DIR}/frame_%04d.png" -vf "pad=1920:1062:(ow-iw)/2:(oh-ih)/2" -c:v libx264 -pix_fmt yuv420p -crf 18 -y {OUTPUT_VIDEO}')
192       else:
193           print("\nFFmpeg not found. Run this command manually:")
194           print(f'ffmpeg -framerate {FRAME_RATE} -i "{OUTPUT_DIR}/frame_%04d.png" -vf "pad=1920:1062:(ow-iw)/2:(oh-ih)/2" -c:v libx264 -pix_fmt yuv420p -crf 18 -y {OUTPUT_VIDEO}')
195
196       print("\n" + "=" * 60)
197       print("PROCESS COMPLETE")
198       print("=" * 60)
199       print(f"Animation saved to: {OUTPUT_VIDEO}")
200       ;;;;
201       %slc_pvpyend;
